...
export class RequestObject {
    ...
    public autoReconnect: number = 0;  // -1 永久重连，0不自动重连，其他正整数为自动Thử lại次数
    public seq:number = 0;             // 消息的序号
    public sended:boolean = false;        // 是否Gửi
...
export interface ISocket {
    ...
    onClosed: (event) => void;              // Đóng回调

    connect(options: any);                  // 连接接口
    send(buffer: NetData);                  // 数据Gửi接口
    close(code?: number, reason?: string);  // Đóng接口
}
