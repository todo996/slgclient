...
export default class UnionLobbyLogic extends Component {
    ...
    private applyModernLobby(): void {
        ...
        ui().suppressLegacyChrome(this.node, 2);
        const info = ui().ensureUiChild(this.node, '__UnionInfoPanel');
        info.setPosition(-420, -5, 0);
        info.setSiblingIndex(0);
        ui().drawAncientPanel(info, 300, 485, 9);
        const title = ui().createUiText(info, '__UnionInfoTitle', 'SỨC MẠNH ĐOÀN KẾT', 19, ui().ANCIENT_UI.gold, 250, 44, true);
        title.node.setPosition(0, 190, 0);
        const text = ui().createUiText(
            info,
            '__UnionInfoText',
        ...
        ui().ensureUiTransform(this.scrollView.node, 780, 485);
        const view = this.scrollView.node.getChildByName('view') || this.scrollView.node.getChildByName('View');
        if (view) {
            ui().ensureUiTransform(view, 780, 485);
        }
        if (this.scrollView.content) {
            ui().ensureUiTransform(this.scrollView.content, 780, 485);
        }
    }
