...
export default class ChatLogic extends Component {
    ...
    private applyModernChat(): void {
        ui().applyAncientScreenChrome(this.node, 'Trò chuyện');

        const channels = ui().ensureUiChild(this.node, '__ChatChannels');
        channels.setPosition(-495, -12, 0);
        channels.setSiblingIndex(this.node.children.length - 2);
        ui().drawAncientPanel(channels, 220, 500, 9);
        const caption = ui().createUiText(
            channels,
            '__ChannelCaption',
            'KÊNH TRÒ CHUYỆN',
            17,
        ...
        const world = ui().findButtonByHandler(this.node, 'onClickWorld');
        if (world) {
            world.node.setParent(channels);
            world.node.setPosition(0, 135, 0);
            ui().styleAncientButton(world.node, 'Thế giới', this._type === 0 ? 'gold' : 'dark', 184, 54);
        }
        const union = ui().findButtonByHandler(this.node, 'onClickUnion');
        if (union) {
            union.node.setParent(channels);
            union.node.setPosition(0, 68, 0);
            ui().styleAncientButton(union.node, 'Liên minh', this._type === 1 ? 'gold' : 'dark', 184, 54);
        }

        this.chatView.node.setPosition(95, 12, 0);
        ui().ensureUiTransform(this.chatView.node, 920, 455);
        const view = this.chatView.node.getChildByName('view') || this.chatView.node.getChildByName('View');
        if (view) {
            ui().ensureUiTransform(view, 920, 455);
        }
        if (this.chatView.content) {
            ui().ensureUiTransform(this.chatView.content, 920, 455);
        }

        this.editConent.node.setParent(this.node);
        this.editConent.node.setPosition(45, -292, 0);
        ui().styleAncientEditBox(this.editConent, 'Nhập nội dung...', 790, 56);
        this.editConent.node.setSiblingIndex(this.node.children.length - 1);

        const send = ui().findButtonByHandler(this.node, 'onClickChat');
        if (send) {
            send.node.setParent(this.node);
            send.node.setPosition(520, -292, 0);
            ui().styleAncientButton(send.node, 'Gửi', 'gold', 150, 56);
