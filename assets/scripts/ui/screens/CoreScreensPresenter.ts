...
function findRosterTitle(root: Node): Label | null {
...
function configureRosterList(scrollView: ScrollView): void {
...
function drawRosterHeader(root: Node): void {
    ...
    const title = findRosterTitle(root) || createGameText(
        header,
        '__RosterTitle',
        'Tướng',
        40,
...
function drawRosterBottomBar(root: Node, countLabel: Label): void {
    ...
    if (convert) {
        ...
        styleRosterButton(convert, 'Sắp xếp', 'jade', 170, 48);
    }
    ...
    if (roster) {
        ...
        styleRosterButton(roster, 'Xem chi tiết', 'primary', 190, 48);
    }
}

function styleRosterBack(root: Node): void {
    ...
    styleRosterButton(close, '←', 'secondary', 72, 52);
}

function applyRosterLayout(root: Node, scrollView: ScrollView, countLabel: Label): void {
    ...
    configureRosterList(scrollView);
    drawRosterHeader(root);
    drawRosterBottomBar(root, countLabel);
    styleRosterBack(root);
}

/** Chỉ nối visual presenter vào component thật; không thay handler, packet hoặc gameplay. */
