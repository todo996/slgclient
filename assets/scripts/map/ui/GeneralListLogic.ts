...
export default class GeneralListLogic extends Component {
    ...
    private applyModernRoster(): void {
        ...
        ui().applyAncientScreenChrome(this.node, 'Tướng');
        ...
        ui().ensureUiTransform(this.scrollView.node, 1110, 510);
        const view = this.scrollView.node.getChildByName('view') || this.scrollView.node.getChildByName('View');
        if (view) {
            ui().ensureUiTransform(view, 1110, 510);
        }
        if (this.scrollView.content) {
            ui().ensureUiTransform(this.scrollView.content, 1110, 510);
        }
        ...
        const convert = ui().findButtonByHandler(this.node, 'onClickConvert');
        if (convert) {
            convert.node.setPosition(330, -319, 0);
            ui().styleAncientButton(convert.node, 'Chuyển đổi', 'jade', 180, 48);
        }

        const roster = ui().findButtonByHandler(this.node, 'onTuJianConvert');
        if (roster) {
            roster.node.setPosition(510, -319, 0);
            ui().styleAncientButton(roster.node, 'Đồ giám', 'gold', 150, 48);
        }
        ...
        if (this.cntLab) {
            ...
            ui().ensureUiTransform(this.cntLab.node, 180, 34);
        }
