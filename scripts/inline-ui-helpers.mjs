...
const ccNames = [
  'Button', 'Color', 'EditBox', 'Graphics', 'HorizontalTextAlignment',
  'Label', 'Node', 'Sprite', 'UITransform', 'VerticalTextAlignment',
];
...
function normalizeCcImport(source) {
  const re = /import\s*\{([\s\S]*?)\}\s*from\s*['"]cc['"];/;
  const match = source.match(re);
  if (!match) throw new Error('cc import not found');
  const names = match[1].split(',').map((value) => value.trim()).filter(Boolean);
  const merged = [...new Set([...names, ...ccNames])];
  const replacement = `import { ${merged.join(', ')} } from 'cc';`;
  return source.replace(re, replacement);
}

function stripSharedUiImports(source) {
  const re = /import\s*\{([\s\S]*?)\}\s*from\s*['"]([^'"]*(?:common\/AudioManager|i18n\/I18n))['"];/g;
  return source.replace(re, (whole, body, modulePath) => {
    const names = body.split(',').map((value) => value.trim()).filter(Boolean);
    const kept = names.filter((name) => !helperNames.has(name));
...
function insertHelpers(source) {
  if (source.includes('const ANCIENT_UI = {')) return source;
  const imports = [...source.matchAll(/import[\s\S]*?from\s*['"][^'"]+['"];/g)];
  if (imports.length === 0) throw new Error('No imports found');
  const last = imports[imports.length - 1];
  const index = last.index + last[0].length;
  return source.slice(0, index) + '\n' + helperBlock + source.slice(index);
}

for (const file of files) {
  let source = fs.readFileSync(file, 'utf8');
  source = stripSharedUiImports(source);
  source = normalizeCcImport(source);
  source = insertHelpers(source);
  fs.writeFileSync(file, source);
  console.log(`Inlined UI helpers: ${file}`);
}

for (const file of files) {
  const source = fs.readFileSync(file, 'utf8');
  if (/from\s*['"][^'"]*(?:common\/AudioManager|i18n\/I18n)['"]/.test(source) && /ANCIENT_UI|applyAncientScreenChrome|drawAncientPanel/.test(source.match(/import[\s\S]*?;/g)?.join('\n') || '')) {
    throw new Error(`Shared helper import remains in ${file}`);
  }
}

console.log(`Completed inline migration for ${files.length} controllers.`);
